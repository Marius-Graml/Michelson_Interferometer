...
class pid_controller():
    ...
    def get_PID_output_all_last(self, input_signal, dith_freq, amp_dith):
    ...
    def get_PID_output_last(self, input_signal, dith_freq, amp_dith):
        data = np.array(input_signal['ch'].iat[-1])
        output_data = self.obj(data)
        output_data = np.reshape(output_data, (-1,1))
        t = np.reshape(input_signal['time'].iat[-1], (-1,1))
        # Add dither signal
        output_data = output_data + amp_dith * np.sin(2*np.pi*dith_freq * t)
        # Add offset to pid_output
        output_data = 1/15 * 150/2 + output_data
        # pd DataFrame
        output_data = np.concatenate((t, output_data), axis=1)
        output_data = np.concatenate((output_data, output_data), axis=0)
        output_data = pd.DataFrame(output_data, columns=['time', 'ch'])
        return output_data